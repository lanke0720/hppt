...
//全局的epoll抽象体，所有协程共享一个。
//处理事件， 网络IO,超时，以及信号量事件
struct stCoEpoll_t  
{
	int iEpollFd; //epoll 监听套接字
	static const int _EPOLL_SIZE = 1024 * 10;//最大事件数

	struct stTimeout_t *pTimeout;//时间定时器

	struct stTimeoutItemLink_t *pstTimeoutList;/* 保存已经超时的事件列表 */

	struct stTimeoutItemLink_t *pstActiveList;/* 保存epoll_wait得到的就绪事件和定时器超时事件 */
...
int co_create( stCoRoutine_t **ppco,const stCoRoutineAttr_t *attr,pfn_co_routine_t pfn,void *arg )
{
	...
	{
		co_init_curr_thread_env();//初始化环境变量
	}

		...
		char *path, char *query_string)
{
    int tmp_ret = 100;
	int content_len = -1;
	char method_env[1024];/*[SIZE/10];*/
	char query_string_env[SIZE*10];
    ...
    {//child
     printf("child .............\n");
        close(input[1]);
		close(output[0]);
        printf("method=%s,1111111111111\n", method);
		sprintf(method_env, "METHOD=%s", method);
        printf("mid ------------------\n");
        printf("method_env::%s", method_env);
		tmp_ret = putenv(method_env);
        printf("%d",tmp_ret);
        printf("exe_cgi::child~~~~~~~~~~~~~~~:METHOD");

		if(strcasecmp(method, "GET") == 0){
                printf("exe_cgi::child~~~~~~~~~~~~~~~:GET");
...
{
	int sock = (int)arg;//#ifdef _DEBUG_
#ifdef _DEBUG_
	char line[1024];
	do{
